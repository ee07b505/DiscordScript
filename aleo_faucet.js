...
const fileJson = JSON.parse(fs.readFileSync("./config/message_aleofaucet.json").toString())
...
const aleoAddresses = fs.readFileSync("./config/aleoaddress.txt").toString().split("\n").filter(Boolean)
...
function getRandomProxy() { 
 ...
 async function main() {
    for (let index = 0; index < aleoAddresses.length; index++) {
        const aleoAddress = aleoAddresses[index];
        const num=index%tokens.length
        const token = tokens[num]
        discord_gm_task(token.trim(),aleoAddress,index)
        // 停止10-30秒
 ...
 async function discord_gm_task(token,aleoAddress,accountNum) {

    let accountToken = token


        let config_index = fileJson.config[0]
        let discordChannelID = config_index.channel_id
        let timeInterval = config_index.time_interval
        let dc_name = config_index.dc_name
        chat(accountNum, discordChannelID, timeInterval, accountToken, dc_name,0,aleoAddress)

        // 停止45-60秒
        let sleep_num = (Math.random() * 15 + 45) * 1000
        console.log(sleep_num)
        await sleep(sleep_num)
...
async function chat(accountNum, discordChannelID, timeInterval, token, dc_name,no_discord,aleoAddress) {
    let index = 0
    while (true) {
        try {
            let messageString = `/sendcredits ${aleoAddress} 50`
